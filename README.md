...
│   └── tests/               testthat suite + fixtures/ synthetic-data generator for CI
├── data/                    WRDS exports (gitignored; must be supplied locally)
├── output/                  Generated figures (static PNG + interactive HTML)
├── .github/workflows/       CI: full SQL + targets + testthat run on synthetic data
...
4. Run the R pipeline:
   ...
   ```
   All 8 `testthat` cases pass against the live database, including an independently-recomputed look-ahead-bias check and boundary-condition tests (insufficient-history stocks correctly excluded; no double-counting in `holding_batches`).

   Steps 2–4 are precisely what CI executes on every push, with one substitution: because real WRDS data cannot be redistributed, CI replaces the gitignored exports with a deterministic synthetic panel generated by `Rscript R/tests/fixtures/make_fixtures.R`. That panel deliberately embeds the edge cases the real data exercises — ETF rows that must be excluded by the universe filter, a short-history security that must produce no `F12_S1` signal, and a verbatim duplicate `(permno, mth)` row that must be collapsed — so CI runs the identical SQL, targets, and testthat code paths. Its numerical results are meaningless and are never reported; CI verifies that the machinery runs, not what the numbers say.
